...
public class CsdnBlogPageProcessor implements PageProcessor {
	...
	private List<CsdnBlog> cblist=new LinkedList<CsdnBlog>();

	public List<CsdnBlog> getCblist() {
		return cblist;
	}

	public void setCblist(List<CsdnBlog> cblist) {
		this.cblist = cblist;
	}
	...
	public void process(Page page) {
		...
		if (!page.getUrl().regex("http://blog\\.csdn\\.net/" + username + "/article/details/\\d+").match()) {
			...
			Selectable tmp=page.getUrl().regex(
					"http://blog\\.csdn\\.net/" + username +
							"/article/details/(\\d+)");


			csdnBlog.setId(Integer.parseInt(tmp.toString()
			));
			// 设置标题
			csdnBlog.setTitle(
					page.getHtml().xpath("//div[@class='article_title']//span[@class='link_title']/a/text()").toString());
			// 设置日期
			csdnBlog.setDate(
					page.getHtml().xpath("//div[@class='article_r']/span[@class='link_postdate']/text()").toString());
			// 设置标签（可以有多个，用,来分割）
			...
			csdnBlog.setView(Integer.parseInt(page.getHtml().xpath("//div[@class='article_r']/span[@class='link_view']")
					.regex("(\\d+)人阅读").toString()));
			// 设置评论人数
			csdnBlog.setComments(Integer.parseInt(page.getHtml()
					.xpath("//div[@class='article_r']/span[@class='link_comments']").regex("\\((\\d+)\\)").toString()));
			// 设置是否原创
			csdnBlog.setCopyright(page.getHtml().regex("bog_copyright").match() ? 1 : 0);

			csdnBlog.setContent(
					page.getHtml().xpath("//div[@class='article_content']").toString());


			System.out.println("con:" + csdnBlog.getContent());
			// 把对象存入数据库
			cblist.add(csdnBlog);
